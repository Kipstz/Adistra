...
for (const x in config.tokens) {
    ...
    if(config.tokens[x].guildID == ''){
        console.log(`^1AdistraLogs Error:^0 Bot ^4${x}^0 is missing a GuildID in de config.`)
        console.log(`^1AdistraLogs Error:^0 Make sure to add this to your config or the bot won't send any messages.`) 
    }
    if(config.tokens[x].token !== ''){
        try{
            client[x].login(config.tokens[x].token).catch(async err => {
                if (err.code === 'DISALLOWED_INTENTS'){
                    await console.log(`^1AdistraLogs Error:^0 Bot ^4${x}^0 Does not have the required Intents.`)
                    await console.log(`^1AdistraLogs Error:^0 Please make sure to add the following Intents to the bot:`)
                    await console.log(`^1AdistraLogs Error:^0 • PRESENCE INTENT`)
                    await console.log(`^1AdistraLogs Error:^0 • SERVER MEMBERS INTENT`)
                    await console.log(`^1AdistraLogs Error:^0 • MESSAGE CONTENT INTENT`)
                } else if(err.code === 'TOKEN_INVALID') {
                    await console.log(`^1AdistraLogs Error:^0 Bot Token for bot ^4${x}^0 is invalid.`)
                }
            })
        } catch {
            console.log(`^1AdistraLogs Error:^0 Could not login with bot token ${x}`)
        }
        client[x].on("ready", async () => {
            let dest = false
            client[x].user.setPresence({ activities: [{ name: `AdistraLogs • Bot Client: ${x}/${Object.keys(config.tokens).length}`, type: "PLAYING" }], afk: true, status: 'dnd' });
            if(x == '1'){
                try{
                    ...
                    for (const i in permissionCheck) {
                        if(!checkGuild.me.permissions.has(permissionCheck[i])){
                            await console.log(`^1AdistraLogs Error:^0 Bot ^4${client[x].user.tag}^0 is missing the ^4${permissionCheck[i]}^0 permission.`)
                            dest = true
                        }
                    };
                } catch {
                    console.log(`^1AdistraLogs Error:^0 Could not verify permissions for ^4${client[x].user.tag}^0. Make sure you added a guild id.`)
                }
            }
            if(dest){
                console.log(`^1AdistraLogs Error:^0 Fix the permissions for ^4${client[x].user.tag}^0 and restart AdistraLogs!`)
            } else {
        ...
        client[x].on("rateLimit", async (data) => {
            ...
            await console.log(`^1AdistraLogs Error:^0Rate limit hit on client: ^4${x}^0`)
            await console.log(`^1AdistraLogs Error:^0Consider adding another bot token to spread load more.`)
            await console.log(`^1AdistraLogs Error:^0Messages will be in a queue until the rate limit ends: ^4${data.timeout/1000}^0 Seconds.`)
        })
    }
}
...
if(GetCurrentResourceName() !== GetInvokingResource()) return 505
    ...
    const embed = new MessageEmbed()
        ...
        if(args.player_1){
            ...
            if(args.player_1['info'].accounts && config.accounts){
                disp = `${disp}\n:cd: **Money:** ${args.player_1['info']['accounts']['money'] ?? 'N/A'}\n:dvd: **Bank:** ${args.player_1['info']['accounts']['bank'] ?? 'N/A'}\n:dvd: **Black Money:** ${args.player_1['info']['accounts']['black_money'] ?? 'N/A'}`
            }
        ...
        if(args.player_2){
            ...
            if(args.player_1['info'].accounts && config.accounts){
                disp = `${disp}\n:cd: **Money:** ${args.player_2['info']['accounts']['money'] ?? 'N/A'}\n:dvd: **Bank:** ${args.player_2['info']['accounts']['bank'] ?? 'N/A'}\n:dvd: **Black Money:** ${args.player_2['info']['accounts']['black_money'] ?? 'N/A'}`
            }
    ...
    if(args.channel === 'system'){
        try {
            ...
            console.log(`^1AdistraLogs Error: ^2${client[args.client].user.tag}^0 Could not send message to ^2system^0 channel`)
        }
        return 200
    } else {
        if(channels[args.channel].embed){
            try {
                ...
                console.log(`^1AdistraLogs Error: ^2${client[args.client].user.tag}^0 Could not send message to ^2${args.channel}^0 channel`)
 
                console.log(`Chanel ^1${args.channel}^0: ^1${channels[args.channel].channelId}^0 ${args.msg}`)
            }           
        } else {
            try {
        ...
        if(channels['all'].embed){
            try {
                ...
                console.log(`^1AdistraLogs Error: ^2${client[args.client].user.tag}^0 Could not send message to ^2all^0 channel`)
            }
        } else {
            try {
