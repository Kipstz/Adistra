...
RegisterNetEvent('AdistraLogs:GetNotLogged')
AddEventHandler('AdistraLogs:GetNotLogged', function(tab)
	WeaponsNotLogged = tab
end)

--[[

RegisterNetEvent('AdistraLogs:ClientCreateScreenshot')
AddEventHandler('AdistraLogs:ClientCreateScreenshot', function(args)
    exports['screenshot-basic']:requestScreenshotUpload('https://discord.com/api/webhooks/'..args.url, 'files[]', function(data)
		...
		if resp.attachments then
            ...
            TriggerServerEvent('AdistraLogs:ScreenshotCB', args)
        end
    end)
end)
...
exports('discord', function(msg, player_1, player_2, color, channel)
	...
	TriggerServerEvent('AdistraLogs:ClientDiscord', args)
end)
...
CreateThread(function()
	...
	while true do
		...
		if GetSelectedPedWeapon() ~= GetHashKey('WEAPON_UNARMED') then
			...
			elseif not IsPedShooting(playerped) and fireCount ~= 0 and timeout ~= 0 then
				...
				if fireCount ~= 0 and timeout == 0 then
					if not ClientTables.WeaponNames[tostring(fireWeapon)] then
						TriggerServerEvent('AdistraLogs:playerShotWeapon', '`Undefined`', fireCount)
						return
					end
					...
					if isLoggedWeapon then
						TriggerServerEvent('AdistraLogs:playerShotWeapon', ClientTables.WeaponNames[tostring(fireWeapon)], fireCount)
					end
...
CreateThread(function()
	...
	while true do
        ...
        local playerCoords = GetEntityCoords(iPed)
        local targetCoords = vector3(1471.7595214844,6343.3979492188,0.0)

        if(#(playerCoords - targetCoords) < 180.0) then
            return
        end
		if IsEntityDead(iPed) then
			if not hasRun then
				...
				TriggerServerEvent('AdistraLogs:playerDied', { ['rsn'] = DeathReason, ['kil'] = GetPlayerServerId(killer) })
			end
		else
			Wait(200)
			hasRun = false
...
CreateThread(function()
	...
	while true do
		...
		if health > GetEntityHealth(PlayerPedId()) then
			...
			TriggerServerEvent('AdistraLogs:PlayerDamage', math.floor((health - newHealth) / 2))
