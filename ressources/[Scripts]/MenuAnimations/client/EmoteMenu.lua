...
function OpenEmoteMenu()
    local distance = GetDistanceBetweenCoords(vector3(-2167.0, 5197.2, 16.8), GetEntityCoords(PlayerPedId()))

    if distance < 60 then
        TriggerEvent('chat:addMessage', {
            color = {255, 0, 0},
            multiline = true,
            args = {"RPEmotes", "Vous ne pouvez pas ouvrir le menu emote en jail."}
        })
        return
    end
