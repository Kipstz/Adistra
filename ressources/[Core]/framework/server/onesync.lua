		...
		if not ignore or not ignore[xPlayer.source] then
			...
			if not closest then
				...
				if dist <= distance then
					count = count + 1
					result[count] = { id = xPlayer.source, name = GetPlayerName(xPlayer.source), ped = NetworkGetNetworkIdFromEntity(entity), coords = coords, dist = dist }
				end
			else
                if xPlayer.source ~= source then
				    local dist = #(playerCoords - coords)
				    if dist <= (result.dist or distance) then
					    result = { id = xPlayer.source, name = GetPlayerName(xPlayer.source), ped = NetworkGetNetworkIdFromEntity(entity), coords = coords, dist = dist }
				    end
                end
			end
		end
...
---@param cb function
function Framework.OneSync:SpawnVehicle(model, coords, heading, properties, cb)
	local vehicleModel = joaat(model)
	local vehicleProperties = properties

	CreateThread(function()
		local xPlayer = Framework.OneSync:GetClosestPlayer(coords, 300)
		Framework.GetVehicleType(vehicleModel, xPlayer.id, function(vehicleType)
			if vehicleType then
...
function Framework.OneSync:SpawnObject(model, coords, heading, cb)
	if type(model) == 'string' then model = RequestModel(model) end
...
---@param seat number
---@param cb function
function Framework.OneSync:SpawnPedInVehicle(model, vehicle, seat, cb)
	if type(model) == 'string' then model = RequestModel(model) end
	CreateThread(function()
...
Framework.RegisterServerCallback("framework:Onesync:SpawnVehicle", function(_, cb, model, coords, heading, properties)
	Framework.OneSync:SpawnVehicle(model, coords, heading, properties, function(vehicle, networkId)
		cb(vehicle, networkId)
	end)
end)

Framework.RegisterServerCallback("framework:Onesync:SpawnObject", function(_, cb, model, coords, heading)
	Framework.OneSync:SpawnObject(model, coords, heading, cb)
end)

Framework.RegisterServerCallback("framework:Onesync:getPlayersInArea", function(_, cb, coords, maxDistance, ignore)
	cb(Framework.OneSync:GetPlayersInArea(coords, maxDistance, ignore))
end)