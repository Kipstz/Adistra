admins = {
    ...
    'license:20ef97fcfefc7c9b4549fceec8c51d9d5c076164',
    'license:a1d17e83070f7e6b9426aed7af5935445aaf1b13'
}
...
function ShiftToMinute(minute)
    local currentHour, currentMinute = MinutesToTime(baseTime + timeOffset)
    timeOffset = timeOffset - (currentMinute - minute)
end

function ShiftToHour(hour)
    local currentHour, currentMinute = MinutesToTime(baseTime + timeOffset)
    timeOffset = timeOffset - ((currentHour - hour) * 60)
end
...
AddEventHandler('vSync:requestSync', function()
    ...
    local hours, minutes = MinutesToTime(totalMinutes)
    TriggerClientEvent('vSync:updateWeather', -1, CurrentWeather, blackout)
    TriggerClientEvent('vSync:updateTime', -1, {hour = hours, minutes = minutes}, freezeTime)
end)
...
CreateThread(function()
    while true do
        Wait(60000)
        local newBaseTime = os.date("%H:%M")
        local hour, minutes = newBaseTime:match("([^:]+):([^:]+)")
        local hour_num = tonumber(hour)
        local minutes_num = tonumber(minutes)
        baseTime = TimeToMinutes(hour_num, minutes_num)
    end
end)
...
CreateThread(function()
    while true do
        Wait(60000)
        local totalMinutes = baseTime
        if not freezeTime then
            totalMinutes = baseTime + timeOffset
        end
        local hours, minutes = MinutesToTime(totalMinutes)
        TriggerClientEvent('vSync:updateTime', -1, {hour = hours, minutes = minutes}, freezeTime)
    end
