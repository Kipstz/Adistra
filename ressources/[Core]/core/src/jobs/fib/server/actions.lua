...
AddEventHandler("FibJob:Menotter", function(player)
    TriggerClientEvent("FibJob:Menotter", player)
end)

RegisterNetEvent("FibJob:Escorter")
AddEventHandler("FibJob:Escorter", function(player)
    TriggerClientEvent("FibJob:Escorter", player, source)
end)

RegisterNetEvent("FibJob:MettreSortirVeh")
AddEventHandler("FibJob:MettreSortirVeh", function(player)
    TriggerClientEvent("FibJob:MettreSortirVeh", player)
end)

Framework.RegisterServerCallback("FibJob:getPlayerData", function(source, cb, player)
...
RegisterNetEvent("FibJob:Confiscate")
AddEventHandler("FibJob:Confiscate", function(player, type, item, quantite)
    local xPlayer = Framework.GetPlayerFromId(source)
    local tPlayer = Framework.GetPlayerFromId(player)
    ...
    if type == 'item' then
        if xPlayer['inventory'].canCarryItem(item.name, quantite) then
            if tPlayer['inventory'].getInventoryItem(item.name).count >= quantite then
                ...
                exports['Logs']:createLog({EmbedMessage = ("[FIB] **%s** à Confisquer **%s**x **%s** à **%s**"):format(GetPlayerName(source), quantite, Framework.ITEMS:GetItemLabel(item.name), GetPlayerName(player)), player_id = source, player_2_id = player, channel = 'interactions'})
            else
        ...
        if tPlayer['loadout'].hasWeapon(item.name) then
            if not xPlayer['loadout'].hasWeapon(item.name) then
                ...
                exports['Logs']:createLog({EmbedMessage = ("[FIB] **%s** à Confisquer **%s** à **%s**"):format(GetPlayerName(source), Framework.GetWeaponLabel(item.name), GetPlayerName(player)), player_id = source, player_2_id = player, channel = 'interactions'})
            else
...
Framework.RegisterServerCallback("FibJob:getVehicleInfos", function(source, cb, plate)
...
RegisterNetEvent("FibJob:Renfort")
AddEventHandler("FibJob:Renfort", function(type, coords)
    ...
    if(canUseRenfort) then
        ...
        for _, xPlayer in pairs(xPlayers) do
            if xPlayer['jobs'] ~= nil then
                if xPlayer['jobs'].getJob().name == 'fib' then
                    xPlayer.triggerEvent("FibJob:setBlip", coords, type)
                end
            end
        end
        Citizen.Wait(8000)
