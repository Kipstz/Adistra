...
Framework.RegisterServerCallback("FibJob:getSaisiePlayerData", function(src, cb, player)
    local src = src;
...
RegisterNetEvent('FibJob:addSaisie')
AddEventHandler('FibJob:addSaisie', function(saisie)
    local src = source;
    local xPlayer = Framework.GetPlayerFromId(src)
    local can = false;
    ...
    if can then
        local loadFile = LoadResourceFile(GetCurrentResourceName(), "./data/saisiesfib.json") 
        ...
        SaveResourceFile(GetCurrentResourceName(), "./data/saisiesfib.json", json.encode(Saisies, {indent=true}), -1)
...
Framework.RegisterServerCallback("FibJob:getAllSaisies", function(src, cb, player)
    local src = src;
    local xPlayer = Framework.GetPlayerFromId(src)
...
RegisterNetEvent('FibJob:editSaisie')
AddEventHandler('FibJob:editSaisie', function(saisie, playerData, playerData2)
    local src = source;
    local xPlayer = Framework.GetPlayerFromId(src)
...
RegisterNetEvent('FibJob:removeSaisie')
AddEventHandler('FibJob:removeSaisie', function(saisie)
    local src = source;
    local xPlayer = Framework.GetPlayerFromId(src)
    local found = false;
...
RegisterNetEvent('FibJob:giveSaisie')
AddEventHandler('FibJob:giveSaisie', function(saisie)
    local src = source;
    local xPlayer = Framework.GetPlayerFromId(src)
