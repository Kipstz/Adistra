CayoAutoJob.VestiaireMenu = RageUI.CreateMenu("Cayo Perico Automobile", "~p~Cayo Perico Automobile~s~: Vestiaire", 8, 200)

local open = false

CayoAutoJob.VestiaireMenu.Closed = function()
    open = false
end

CayoAutoJob.OpenVestiaireMenu = function()
    if open then
        RageUI.CloseAll()
        RageUI.Visible(CayoAutoJob.VestiaireMenu, false)
        open = false
    else
        RageUI.CloseAll()
        RageUI.Visible(CayoAutoJob.VestiaireMenu, true)
        open = true
        ...
        CreateThread(function()
            while open do
                ...
                RageUI.IsVisible(CayoAutoJob.VestiaireMenu, true, true, true, function()
                    ...
                    for k,v in pairs(Config['job_cayoauto'].Tenues) do
                    ...
                    for k,v in pairs(Config['job_cayoauto'].Sacs) do
                        RageUI.ButtonWithStyle(v.label, nil, { RightLabel = "→" }, true, function(Hovered, Active, Selected)
                            if Selected then
                                Framework.TriggerServerCallback('skinchanger:getPlayerSkin', function(skin)
                                    if skin.sex == 0 then
