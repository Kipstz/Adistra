...
NightClub = {}
NightClub.inTimeout = false

RegisterServerEvent('NightClub:annonce')
AddEventHandler('NightClub:annonce', function(annonce)
    local xPlayer = Framework.GetPlayerFromId(source)

    if not NightClub.inTimeout then
        local xPlayers	= Framework.GetPlayers()
        for i=1, #xPlayers, 1 do
            TriggerClientEvent('framework:showAdvancedNotification', xPlayers[i], "77-Club", '~b~Annonce', annonce)
        end

        NightClub.startTimeout(60000 * 5)
    else
        xPlayer.showNotification("~r~Veuillez patienter avant de faire une autre annonce (5 min entre chaque annonce) !")
    end
end)

RegisterServerEvent('NightClub:buy')
AddEventHandler('NightClub:buy', function(item, qte)
    local xPlayer = Framework.GetPlayerFromId(source)
...
RegisterServerEvent('NightClub:craft')
AddEventHandler('NightClub:craft', function(needs, item)
    local xPlayer = Framework.GetPlayerFromId(source)
...
NightClub.startTimeout = function(time)
    NightClub.inTimeout = true
