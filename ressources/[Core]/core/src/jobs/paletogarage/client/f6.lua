...
PaletoGarageJob.F6Menu = RageUI.CreateMenu("Paleto Garage", "~p~Paleto Garage~s~: Actions", 8, 200)

PaletoGarageJob.InteractionsCitoyens = RageUI.CreateSubMenu(PaletoGarageJob.F6Menu, "Paleto Garage", "~p~Paleto Garage~s~: Interactions Citoyens", 8, 200)

PaletoGarageJob.InteractionsVehicules = RageUI.CreateSubMenu(PaletoGarageJob.F6Menu, "Paleto Garage", "~p~Paleto Garage~s~: Interactions Vehicules", 8, 200)

PaletoGarageJob.GestionsObjets = RageUI.CreateSubMenu(PaletoGarageJob.F6Menu, "Paleto Garage", "~p~Paleto Garage~s~: Gestion Objets", 8, 200)

PaletoGarageJob.AutresMenu = RageUI.CreateSubMenu(PaletoGarageJob.F6Menu, "Paleto Garage", "~p~Paleto Garage~s~: Autres", 8, 200)

local open = false

PaletoGarageJob.F6Menu.Closed = function()
    open = false
end

PaletoGarageJob.ServiceCheck = false

PaletoGarageJob.OpenF6Menu = function()
    if open then
        RageUI.CloseAll()
        RageUI.Visible(PaletoGarageJob.F6Menu, false)
        open = false
    else
        RageUI.CloseAll()
        RageUI.Visible(PaletoGarageJob.F6Menu, true)
        open = true
        ...
        CreateThread(function()
            while open do
                ...
                RageUI.IsVisible(PaletoGarageJob.F6Menu, true, true, true, function()
                    RageUI.Checkbox("Prise de Service", nil, PaletoGarageJob.ServiceCheck, {}, function(Hovered, Active, Selected, Checked)
                        if Selected then
                            PaletoGarageJob.ServiceCheck = Checked
                            ...
                            if Checked then        
                                Framework.TriggerServerCallback("service:enableService", function(canTakeService, maxInService, inServiceCount)
                                    if canTakeService then
                                        ...
                                        TriggerServerEvent("service:notifyAllInService", notification, 'paletogarage')
                                    else
                                ...
                                TriggerServerEvent("service:disableService", 'paletogarage')
                            end
                        end
                    end)

                    if PaletoGarageJob.ServiceCheck then
                        ...
                        end, PaletoGarageJob.InteractionsCitoyens)
                        ...
                        end, PaletoGarageJob.InteractionsVehicules)
    
                        RageUI.ButtonWithStyle("Gestions Objets", nil, { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                            if Selected then
                            end
                        end, PaletoGarageJob.GestionsObjets)
    
                        RageUI.ButtonWithStyle("Autres", nil, { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                            if Selected then
                            end
                        end, PaletoGarageJob.AutresMenu)
                    end

                end)

                RageUI.IsVisible(PaletoGarageJob.InteractionsCitoyens, true, true, true, function()
                    local myCoords = GetEntityCoords(PlayerPedId())
                    local closestPlayer, closestPlayerDistance = Framework.Game.GetClosestPlayer(myCoords)
                    ...
                    RageUI.ButtonWithStyle("Mettre une facture", nil, { RightLabel = "→" }, true, function(Hovered, Active, Selected)
                        if Selected then
                            ...
                            if tostring(raison) and tonumber(amount) then
                                exports["ac"]:ExecuteServerEvent('billing:sendBill', GetPlayerServerId(closestPlayer), 'paletogarage', raison, amount)
                            else
                ...
                RageUI.IsVisible(PaletoGarageJob.InteractionsVehicules, true, true, true, function()
                    local myCoords = GetEntityCoords(PlayerPedId())
                    local closestVeh, closestVehDistance = Framework.Game.GetClosestVehicle(myCoords)
                ...
                RageUI.IsVisible(PaletoGarageJob.GestionsObjets, true, true, true, function()
