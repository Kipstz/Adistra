PaletoGarageJob.VestiaireMenu = RageUI.CreateMenu("Paleto Garage", "~p~Paleto Garage~s~: Vestiaire", 8, 200)

local open = false

PaletoGarageJob.VestiaireMenu.Closed = function()
    open = false
end

PaletoGarageJob.OpenVestiaireMenu = function()
    if open then
        RageUI.CloseAll()
        RageUI.Visible(PaletoGarageJob.VestiaireMenu, false)
        open = false
    else
        RageUI.CloseAll()
        RageUI.Visible(PaletoGarageJob.VestiaireMenu, true)
        open = true
        ...
        CreateThread(function()
            while open do
                ...
                RageUI.IsVisible(PaletoGarageJob.VestiaireMenu, true, true, true, function()
                    ...
                    for k,v in pairs(Config['job_paletogarage'].Tenues) do
                    ...
                    for k,v in pairs(Config['job_paletogarage'].Sacs) do
                        RageUI.ButtonWithStyle(v.label, nil, { RightLabel = "→" }, true, function(Hovered, Active, Selected)
                            if Selected then
                                Framework.TriggerServerCallback('skinchanger:getPlayerSkin', function(skin)
                                    if skin.sex == 0 then
