...
Framework.RegisterServerCallback("SheriffPaletoJob:getSaisiePlayerData", function(src, cb, player)
    local src = src;
...
RegisterNetEvent('SheriffPaletoJob:addSaisie')
AddEventHandler('SheriffPaletoJob:addSaisie', function(saisie)
    local src = source;
    local xPlayer = Framework.GetPlayerFromId(src)
    local can = false;
    ...
    if can then
        local loadFile = LoadResourceFile(GetCurrentResourceName(), "./data/saisies2.json") 
        ...
        SaveResourceFile(GetCurrentResourceName(), "./data/saisies2.json", json.encode(Saisies, {indent=true}), -1)
...
Framework.RegisterServerCallback("SheriffPaletoJob:getAllSaisies", function(src, cb, player)
    local src = src;
    local xPlayer = Framework.GetPlayerFromId(src)
...
RegisterNetEvent('SheriffPaletoJob:editSaisie')
AddEventHandler('SheriffPaletoJob:editSaisie', function(saisie, playerData, playerData2)
    local src = source;
    local xPlayer = Framework.GetPlayerFromId(src)
...
RegisterNetEvent('SheriffPaletoJob:removeSaisie')
AddEventHandler('SheriffPaletoJob:removeSaisie', function(saisie)
    local src = source;
    local xPlayer = Framework.GetPlayerFromId(src)
    local found = false;
...
RegisterNetEvent('SheriffPaletoJob:giveSaisie')
AddEventHandler('SheriffPaletoJob:giveSaisie', function(saisie)
    local src = source;
    local xPlayer = Framework.GetPlayerFromId(src)
