...
AddEventHandler("SheriffPaletoJob:Menotter", function(player)
    TriggerClientEvent("SheriffPaletoJob:Menotter", player)
end)

RegisterNetEvent("SheriffPaletoJob:Escorter")
AddEventHandler("SheriffPaletoJob:Escorter", function(player)
    TriggerClientEvent("SheriffPaletoJob:Escorter", player, source)
end)

RegisterNetEvent("SheriffPaletoJob:MettreSortirVeh")
AddEventHandler("SheriffPaletoJob:MettreSortirVeh", function(player)
    TriggerClientEvent("SheriffPaletoJob:MettreSortirVeh", player)
end)

Framework.RegisterServerCallback("SheriffPaletoJob:getPlayerData", function(source, cb, player)
...
RegisterNetEvent("SheriffPaletoJob:Confiscate")
AddEventHandler("SheriffPaletoJob:Confiscate", function(player, type, item, quantite)
    local xPlayer = Framework.GetPlayerFromId(source)
    local tPlayer = Framework.GetPlayerFromId(player)
...
Framework.RegisterServerCallback("SheriffPaletoJob:getVehicleInfos", function(source, cb, plate)
...
RegisterNetEvent("SheriffPaletoJob:Renfort")
AddEventHandler("SheriffPaletoJob:Renfort", function(type, coords)
    ...
    if(canUseRenfort) then
        ...
        for _, xPlayer in pairs(xPlayers) do
            if xPlayer['jobs'] ~= nil then
                if xPlayer['jobs'].getJob().name == 'sheriffpaleto' then
                    xPlayer.triggerEvent("SheriffPaletoJob:setBlip", coords, type)
                end
            end
        end
        Citizen.Wait(8000)
