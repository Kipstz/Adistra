...
PericoBarJob = {}
PericoBarJob.inTimeout = false

RegisterServerEvent('PericoBarJob:annonce')
AddEventHandler('PericoBarJob:annonce', function(annonce)
    local xPlayer = Framework.GetPlayerFromId(source)

    if not PericoBarJob.inTimeout then
        local xPlayers	= Framework.GetPlayers()
        for i=1, #xPlayers, 1 do
            TriggerClientEvent('framework:showAdvancedNotification', xPlayers[i], "Perico-Bar", '~b~Annonce', annonce)
        end

        PericoBarJob.startTimeout(60000 * 5)
    else
        xPlayer.showNotification("~r~Veuillez patienter avant de faire une autre annonce (5 min entre chaque annonce) !")
    end
end)

RegisterServerEvent('PericoBarJob:buy')
AddEventHandler('PericoBarJob:buy', function(item, qte)
    local xPlayer = Framework.GetPlayerFromId(source)
...
RegisterServerEvent('PericoBarJob:craft')
AddEventHandler('PericoBarJob:craft', function(needs, item)
    local xPlayer = Framework.GetPlayerFromId(source)
...
PericoBarJob.startTimeout = function(time)
    PericoBarJob.inTimeout = true
