MecanoJob.VestiaireMenu = RageUI.CreateMenu("Mécano", "~p~Mécano~s~: Vestiaire", 8, 200)

local open = false

MecanoJob.VestiaireMenu.Closed = function()
    open = false
end

MecanoJob.OpenVestiaireMenu = function()
    if open then
        RageUI.CloseAll()
        RageUI.Visible(MecanoJob.VestiaireMenu, false)
        open = false
    else
        RageUI.CloseAll()
        RageUI.Visible(MecanoJob.VestiaireMenu, true)
        open = true
        ...
        CreateThread(function()
            while open do
                ...
                RageUI.IsVisible(MecanoJob.VestiaireMenu, true, true, true, function()
                    ...
                    for k,v in pairs(Config['job_mecano'].Tenues) do
                    ...
                    for k,v in pairs(Config['job_mecano'].Sacs) do
                        RageUI.ButtonWithStyle(v.label, nil, { RightLabel = "→" }, true, function(Hovered, Active, Selected)
                            if Selected then
                                Framework.TriggerServerCallback('skinchanger:getPlayerSkin', function(skin)
                                    if skin.sex == 0 then
