...
BennysJob.F6Menu = RageUI.CreateMenu("Benny's", "~p~Benny's~s~: Actions", 8, 200)

BennysJob.InteractionsCitoyens = RageUI.CreateSubMenu(BennysJob.F6Menu, "Benny's", "~p~Benny's~s~: Interactions Citoyens", 8, 200)

BennysJob.InteractionsVehicules = RageUI.CreateSubMenu(BennysJob.F6Menu, "Benny's", "~p~Benny's~s~: Interactions Vehicules", 8, 200)

BennysJob.GestionsObjets = RageUI.CreateSubMenu(BennysJob.F6Menu, "Benny's", "~p~Benny's~s~: Gestion Objets", 8, 200)

BennysJob.AutresMenu = RageUI.CreateSubMenu(BennysJob.F6Menu, "Benny's", "~p~Benny's~s~: Autres", 8, 200)

local open = false

BennysJob.F6Menu.Closed = function()
    open = false
end

BennysJob.ServiceCheck = false

BennysJob.OpenF6Menu = function()
    if open then
        RageUI.CloseAll()
        RageUI.Visible(BennysJob.F6Menu, false)
        open = false
    else
        RageUI.CloseAll()
        RageUI.Visible(BennysJob.F6Menu, true)
        open = true
        ...
        CreateThread(function()
            while open do
                ...
                RageUI.IsVisible(BennysJob.F6Menu, true, true, true, function()
                    RageUI.Checkbox("Prise de Service", nil, BennysJob.ServiceCheck, {}, function(Hovered, Active, Selected, Checked)
                        if Selected then
                            BennysJob.ServiceCheck = Checked
                            ...
                            if Checked then        
                                Framework.TriggerServerCallback("service:enableService", function(canTakeService, maxInService, inServiceCount)
                                    if canTakeService then
                                        ...
                                        TriggerServerEvent("service:notifyAllInService", notification, 'bennys')
                                    else
                                ...
                                TriggerServerEvent("service:disableService", 'bennys')
                            end
                        end
                    end)

                    if BennysJob.ServiceCheck then
                        ...
                        end, BennysJob.InteractionsCitoyens)
                    ...
                    RageUI.ButtonWithStyle("Mettre une facture", nil, { RightLabel = "→" }, true, function(Hovered, Active, Selected)
                        if Selected then
                            ...
                            if tostring(raison) and tonumber(amount) then
                                exports["ac"]:ExecuteServerEvent('billing:sendBill', GetPlayerServerId(closestPlayer), 'bennys', raison, amount)
                            else
                ...
                RageUI.IsVisible(BennysJob.InteractionsVehicules, true, true, true, function()
                    local myCoords = GetEntityCoords(PlayerPedId())
                    local closestVeh, closestVehDistance = Framework.Game.GetClosestVehicle(myCoords)
                ...
                RageUI.IsVisible(BennysJob.GestionsObjets, true, true, true, function()
