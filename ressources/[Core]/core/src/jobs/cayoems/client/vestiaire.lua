CayoEmsJob.VestiaireMenu = RageUI.CreateMenu("Ambulance", "~p~Ambulance~s~: Vestiaire", 8, 200)

local open = false

CayoEmsJob.VestiaireMenu.Closed = function()
    open = false
end

CayoEmsJob.OpenVestiaireMenu = function()
    if open then
        RageUI.CloseAll()
        RageUI.Visible(CayoEmsJob.VestiaireMenu, false)
        open = false
    else
        RageUI.CloseAll()
        RageUI.Visible(CayoEmsJob.VestiaireMenu, true)
        open = true
        ...
        CreateThread(function()
            while open do
                ...
                RageUI.IsVisible(CayoEmsJob.VestiaireMenu, true, true, true, function()
                    ...
                    for k,v in pairs(Config['job_cayoems'].Tenues) do
                    ...
                    for k,v in pairs(Config['job_cayoems'].Sacs) do
                        RageUI.ButtonWithStyle(v.label, nil, { RightLabel = "→" }, true, function(Hovered, Active, Selected)
                            if Selected then
                                Framework.TriggerServerCallback('skinchanger:getPlayerSkin', function(skin)
                                    if skin.sex == 0 then
