...
TriggerEvent("service:activateService", 'cayoems', 15)

RegisterNetEvent("CayoEmsJob:MettreSortirVeh")
AddEventHandler("CayoEmsJob:MettreSortirVeh", function(player)
    if player ~= nil then
        TriggerClientEvent("CayoEmsJob:MettreSortirVeh", player)
    else
...
AddEventHandler('framework:restoreCharacter', function(source)
	...
	local loadFile = LoadResourceFile(GetCurrentResourceName(), "./data/pharmacie2.json") 

	Pharmacie = json.decode(loadFile)

    if xPlayer ~= nil then
        if xPlayer['jobs'] and xPlayer['jobs'].job.name == 'cayoems' then
            xPlayer.triggerEvent("CayoEmsJob:updateStock", Pharmacie)
        end
    end
end)

RegisterNetEvent("CayoEmsJob:add")
AddEventHandler("CayoEmsJob:add", function(item, qte)
    ...
    if xPlayer['jobs'] and xPlayer['jobs'].job.name == 'cayoems' then
        ...
        SaveResourceFile(GetCurrentResourceName(), "./data/pharmacie2.json", json.encode(Pharmacie, {indent=true}), -1) 
        
        for _,xPlayer in pairs(Framework.GetExtendedPlayers()) do
            if xPlayer and xPlayer['jobs'] and xPlayer['jobs'].job.name == 'cayoems' then
                xPlayer.triggerEvent("CayoEmsJob:updateStock", Pharmacie)
            end
        end
    else
...
RegisterNetEvent("CayoEmsJob:remove")
AddEventHandler("CayoEmsJob:remove", function(item, qte)
	local xPlayer = Framework.GetPlayerFromId(source)
...
RegisterNetEvent("CayoEmsJob:buy")
AddEventHandler("CayoEmsJob:buy", function(item, qte, price)
	local xPlayer = Framework.GetPlayerFromId(source)

    if xPlayer['jobs'] and xPlayer['jobs'].job.name == 'cayoems' then
        TriggerEvent('bossManagement:getSocietyAccount', 'cayoems', function(accountMoney)
            if accountMoney >= price then
                TriggerEvent('bossManagement:removeMoneybyOther', 'cayoems', price)
