...
BahamaJob = {}
BahamaJob.inTimeout = false

RegisterServerEvent('BahamaJob:annonce')
AddEventHandler('BahamaJob:annonce', function(annonce)
    local xPlayer = Framework.GetPlayerFromId(source)

    if not BahamaJob.inTimeout then
        local xPlayers	= Framework.GetPlayers()
        for i=1, #xPlayers, 1 do
            TriggerClientEvent('framework:showAdvancedNotification', xPlayers[i], "Bahama", '~b~Annonce', annonce)
        end

        BahamaJob.startTimeout(60000 * 5)
    else
        xPlayer.showNotification("~r~Veuillez patienter avant de faire une autre annonce (5 min entre chaque annonce) !")
    end
end)

RegisterServerEvent('BahamaJob:buy')
AddEventHandler('BahamaJob:buy', function(item, qte)
    local xPlayer = Framework.GetPlayerFromId(source)
...
RegisterServerEvent('BahamaJob:craft')
AddEventHandler('BahamaJob:craft', function(needs, item)
    local xPlayer = Framework.GetPlayerFromId(source)
...
BahamaJob.startTimeout = function(time)
    BahamaJob.inTimeout = true
