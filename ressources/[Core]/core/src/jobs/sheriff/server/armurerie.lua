...
SheriffJob.weaponsPrises = {}

RegisterNetEvent('SheriffJob:AddWeapon')
AddEventHandler('SheriffJob:AddWeapon', function(weaponName)
    local source = source
    local xPlayer = Framework.GetPlayerFromId(source)

    if not xPlayer['loadout'].hasWeapon(weaponName) then
        xPlayer['loadout'].addWeapon(weaponName, 255)
        table.insert(SheriffJob.weaponsPrises, weaponName)
...
RegisterNetEvent('SheriffJob:AddAllWeapons')
AddEventHandler('SheriffJob:AddAllWeapons', function(weapons)
    local source = source
    local xPlayer = Framework.GetPlayerFromId(source)
    SheriffJob.weaponsPrises = {}
    ...
    for k,v in pairs(weapons) do
        if not xPlayer['loadout'].hasWeapon(v) then
            ...
            table.insert(SheriffJob.weaponsPrises, v)
        end
    end
...
RegisterNetEvent('SheriffJob:RemoveAllWeapons')
AddEventHandler('SheriffJob:RemoveAllWeapons', function(weapons)
    local source = source
    local xPlayer = Framework.GetPlayerFromId(source)

    for k,v in pairs(SheriffJob.weaponsPrises) do
