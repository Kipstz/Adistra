...
SheriffJob.F6Menu = RageUI.CreateMenu("Sheriff", "~p~Sheriff~s~: Actions", 8, 200)

SheriffJob.InteractionsCitoyens = RageUI.CreateSubMenu(SheriffJob.F6Menu, "Sheriff", "~p~Sheriff~s~: Interactions Citoyens", 8, 200)
SheriffJob.FacturesMenu = RageUI.CreateSubMenu(SheriffJob.InteractionsCitoyens, "Sheriff", "~p~Sheriff~s~: Factures", 8, 200)
SheriffJob.FouilleMenu = RageUI.CreateSubMenu(SheriffJob.InteractionsCitoyens, "Sheriff", "~p~Sheriff~s~: Fouille", 8, 200)

SheriffJob.InteractionsVehicules = RageUI.CreateSubMenu(SheriffJob.F6Menu, "Sheriff", "~p~Sheriff~s~: Interactions Vehicules", 8, 200)
SheriffJob.VehicleInfosMenu = RageUI.CreateSubMenu(SheriffJob.InteractionsVehicules, "Sheriff", "~p~Sheriff~s~: Informations du Véhicule", 8, 200)

SheriffJob.GestionsObjets = RageUI.CreateSubMenu(SheriffJob.F6Menu, "Sheriff", "~p~Sheriff~s~: Gestion Objets", 8, 200)

SheriffJob.AutresMenu = RageUI.CreateSubMenu(SheriffJob.F6Menu, "Sheriff", "~p~Sheriff~s~: Autres", 8, 200)
...
SheriffJob.F6Menu.Closed = function()
    open = false
end

SheriffJob.ServiceCheck = false

SheriffJob.OpenF6Menu = function()
    if open then
        RageUI.CloseAll()
        RageUI.Visible(SheriffJob.F6Menu, false)
        open = false
    else
        RageUI.CloseAll()
        RageUI.Visible(SheriffJob.F6Menu, true)
        open = true
        ...
        CreateThread(function()
            while open do
                ...
                RageUI.IsVisible(SheriffJob.F6Menu, true, true, true, function()
                    RageUI.Checkbox("Prise de Service", nil, SheriffJob.ServiceCheck, {}, function(Hovered, Active, Selected, Checked)
                        if Selected then
                            SheriffJob.ServiceCheck = Checked
                            ...
                            if Checked then        
                                Framework.TriggerServerCallback("service:enableService", function(canTakeService, maxInService, inServiceCount)
                                    if canTakeService then
                                        ...
                                        TriggerServerEvent("service:notifyAllInService", notification, 'sheriff')
                                    else
                                ...
                                TriggerServerEvent("service:notifyAllInService", notification, 'sheriff')
                                TriggerServerEvent("service:disableService", 'sheriff')
                            end
                        end
                    end)

                    if SheriffJob.ServiceCheck then
                        ...
                        end, SheriffJob.InteractionsCitoyens)
                ...
                RageUI.IsVisible(SheriffJob.InteractionsCitoyens, true, true, true, function()
                    ...
                    RageUI.ButtonWithStyle("Fouiller", "~r~La personne doit avoir les mains levées !", { RightLabel = "→" }, handsUp, function(Hovered, Active, Selected)
                        if Selected then
                            Framework.TriggerServerCallback("SheriffJob:getPlayerData", function(data) 
                                playerData = data
                            end, GetPlayerServerId(closestPlayer))
                        end
                    end, SheriffJob.FouilleMenu)
                    ...
                    RageUI.ButtonWithStyle("Mettre une amende", nil, { RightLabel = "→" }, true, function(Hovered, Active, Selected)
                        if Selected then
                            ...
                            if tostring(raison) and tonumber(amount) then
                                exports["ac"]:ExecuteServerEvent('billing:sendBill', GetPlayerServerId(closestPlayer), 'sheriff', raison, amount)
                            else
                ...
                RageUI.IsVisible(SheriffJob.FacturesMenu, true, true, true, function()
                ...
                RageUI.IsVisible(SheriffJob.FouilleMenu, true, true, true, function()
                            ...
                            RageUI.ButtonWithStyle(v.itemLabel, "~r~ENTER ~s~pour confisquer", { RightLabel = "~b~x"..v.itemCount }, true, function(Hovered, Active, Selected)
                                if Selected then
                                    ...
                                    if tonumber(quantite) and v.itemCount >= tonumber(quantite) then
                                        ...
                                        exports["ac"]:ExecuteServerEvent("SheriffJob:Confiscate", playerData.src, 'item', { name = v.itemName, label = v.itemLabel, ammo = v.itemCount}, tonumber(quantite))
                                    else
                            ...
                            RageUI.ButtonWithStyle(v.weaponLabel, "~r~ENTER ~s~pour confisquer", { RightLabel = "~o~"..v.weaponAmmo.."Mun" }, true, function(Hovered, Active, Selected)
                                if Selected then
                                    if not Framework.WeaponisBoutique(v.weaponName) then
                                        exports["ac"]:ExecuteServerEvent("SheriffJob:Confiscate", playerData.src, 'weapon', { name = v.weaponName, label = v.weaponLabel, ammo = v.weaponAmmo})
                ...
                RageUI.IsVisible(SheriffJob.InteractionsVehicules, true, true, true, function()
                    ...
                    if closestVeh ~= -1 and closestVehDistance < 5 then
                        ...
                        RageUI.ButtonWithStyle("Informations du Véhicule", nil, {}, true, function(Hovered, Active, Selected)
                            if Selected then
                                ...
                                Framework.TriggerServerCallback("SheriffJob:getVehicleInfos", function(info)
                                    retrivedInfo = info
                                end, vehicleProps.plate)
                            end
                        end, SheriffJob.VehicleInfosMenu)
                ...
                RageUI.IsVisible(SheriffJob.VehicleInfosMenu, true, true, true, function()
                ...
                RageUI.IsVisible(SheriffJob.GestionsObjets, true, true, true, function()
                ...
                RageUI.IsVisible(SheriffJob.AutresMenu, true, true, true, function()
                    ...
                    RageUI.ButtonWithStyle("Code 1", nil, {}, canUseRenfort, function(Hovered, Active, Selected)
                        if Selected then
                            canUseRenfort = false
                            TriggerServerEvent("SheriffJob:Renfort", 'legere', GetEntityCoords(PlayerPedId()))
                            canReuseRenfort()
                        end
                    end)
                    ...
                    RageUI.ButtonWithStyle("Code 2", nil, {}, canUseRenfort, function(Hovered, Active, Selected)
                        if Selected then
                            canUseRenfort = false
                            TriggerServerEvent("SheriffJob:Renfort", 'importante', GetEntityCoords(PlayerPedId()))
                            canReuseRenfort()
                        end
                    end)
                    ...
                    RageUI.ButtonWithStyle("Code 3", nil, {}, canUseRenfort, function(Hovered, Active, Selected)
                        if Selected then
                            canUseRenfort = false
                            TriggerServerEvent("SheriffJob:Renfort", 'urgente', GetEntityCoords(PlayerPedId()))
                            canReuseRenfort()
                        end
                    end)
...
SheriffJob.ActiveBouclier = function()
...
AddEventHandler("SheriffJob:Menotter", function()
...
AddEventHandler("SheriffJob:Escorter", function(copId)
    local playerPed
	local targetPed
...
AddEventHandler("SheriffJob:MettreSortirVeh", function()
    local PlayerPed = PlayerPedId()
...
RegisterNetEvent("SheriffJob:setBlip")
AddEventHandler("SheriffJob:setBlip", function(coords, raison)
